...
def get_productivity_report_data(**kwargs):
    ...
    tx_plan_start_date = today - timedelta(days=210)
    da_start_date = today - timedelta(days=400)
    staff_search_data = {
        'emp_status_f': 'ACTIVE',
        'first_name': 1,
        'last_name': 1,
    ...
    tx_plan_args.update({
        'visittype_id': 3,
        'wh_val1': tx_plan_start_date.strftime(credible_date_format),
    })
    da_args = encounter_search_data.copy()
    da_args.update({
        'visittype_id': 5,
        'wh_val1': da_start_date.strftime(credible_date_format),
    })
...
def _build_not_seen_report(caseloads, encounter_data):
    ...
    results = [
        [
            'Team', 'CSW Name', 'Client ID', 'Last Service by CSW', 'Last Billable Service',
            '30/60/90 per CSW', '30/60/90 per Last Billable'
        ]
    ]
    inverted = _invert_caseloads(caseloads)
