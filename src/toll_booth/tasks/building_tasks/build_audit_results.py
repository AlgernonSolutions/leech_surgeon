...
def build_audit_results(teams, patients, audit_results):
    ...
    for entry in audit_results:
        ...
        provider_name = team_data[1]
        if team_name not in formatted:
            formatted[team_name] = {}
        if provider_name not in formatted[team_name]:
            formatted[team_name][provider_name] = []
        findings = 'nothing found'
        if entry.findings:
            findings = '\n'.join(str(x) for x in entry.findings)
        patient = patients.get(patient_id, 'unknown')
        formatted_row = {
        ...
        formatted[team_name][provider_name].append(formatted_row)
    return formatted
